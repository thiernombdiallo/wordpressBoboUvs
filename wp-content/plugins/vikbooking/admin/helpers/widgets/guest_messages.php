...
class VikBookingAdminWidgetGuestMessages extends VikBookingAdminWidget
{
	...
	{
		$bid_convo = VikRequest::getString('bid_convo', '', 'request');
		...
		if ($bid_convo) {
			...
			if (!$bubble_convo && class_exists('VCMChatMessaging')) {
				// updated VCM versions will allow us to fetch one conversation by booking ID
				$booking_messages = VCMChatMessaging::getInstance()->loadBookingGuestThreads($bid_convo);
				if ($booking_messages) {
